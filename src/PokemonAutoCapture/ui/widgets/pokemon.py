...
class PokemonData(QObject):
    ...
    """
    # ポケモン画像のフォルダー
    pokemon_icons_path = DataConfigClass.get_resource_path("assets", "images", "Pokemon Icons")
    try:
    ...
    def set_pokemon(self, label):
        ...
        # 画像データをセット
        image_path = os.path.join(PokemonData.pokemon_icons_path, str(pokemon_data_row["image_file"].iloc[0]))
